...
#ifndef PIXEL_COUNT
#   define PIXEL_COUNT 4 // Num of pixels in strip
#endif
//
//
...
static VIZ_MODE g_current_mode = VIZ_MODE::SOLID; 
...
static bool g_standby_mode = false; 
static uint32_t g_prev_standby_time;
...
#ifdef DEBUG 
    g_current_mode = VIZ_MODE::SOLID; 
    g_brightness = 255; 
    g_color_idx = 0; 
#else
//  if (EEPROM.length() > EEPROM_mode_address)  // Only try to use EEPROM if there is some on the board
//  {
//      g_current_mode = VIZ_MODE(EEPROM.read(EEPROM_mode_address)); // Restore the mode
//  }
//
//  if (EEPROM.length() > EEPROM_brightness_address)
//  {
//      g_brightness = EEPROM.read(EEPROM_brightness_address);
//  }
//
//  if (EEPROM.length() > EEPROM_color_address)
//  {
//      g_color_idx = EEPROM.read(EEPROM_color_address);
//  }
...
//  ble_setup();
    ...
    g_prev_time = millis(); 
    g_prev_standby_time = g_prev_time; 
...
void rainbow_mode(int multi)
{
    // solid rainbow color
    for (uint16_t i=0; i < strip.numPixels(); i++) {
        uint32_t c = Wheel( i*multi+g_color_idx );
